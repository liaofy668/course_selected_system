<template>
  <div class="grade-wrap">
    ...
    <div class="container">
      ...
      <el-dialog :visible.sync="editing" title="成绩打分" width="30%">
...
export default {
  ...
  methods: {
    query() {
      api
        .getPageCount(this.queryForm.courseName, this.queryForm.studentName,this.$store.state.status.userId)
        .then(res => {
        ...
        .then(res => {
          ...
          for (var i = 0; i <= Object.keys(res).length; i++) {
            var obj = {
              ...
              courseName      : res[i].scCourseName,
              studentName     : res[i].scStudentName,
              score           : res[i].scScore,
            };
    ...
    edit(id) {
      api.get(id).then(res => {
        this.entityForm.studentCourseId = res.scId;
        this.entityForm.score = res.scScore;
        this.editing = true;
      });
    },
    save() {
